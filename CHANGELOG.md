...
- Alpha 16 is an unpublished internal candidate that advances the MSI `ProductVersion` to `2.0.31` for a controlled in-place OSD Server v2 upgrade. It fixes the familiar Guided Setup regression: snapshot steps are selectable and expose their mapped v2 action again. A missing or blocking preflight now offers Run preflight in both the guided rail and ingress gate, while every stopped ingress start control stays disabled until preflight passes.
- Alpha 15 is an unpublished internal candidate for the v1-familiar UX on the v2 security architecture. It advances the MSI `ProductVersion` to `2.0.30` for a controlled in-place OSD Server v2 upgrade while retaining v1 as a protected migration source.
...
- Rebuilt the v2 React shell around the familiar v1 Deploy / Monitor workflow while retaining Fastify, the authenticated named pipe, versioned schemas, OperationCoordinator, SQLite and DPAPI boundaries. The UI now provides readable service cards, Fleet/evidence detail, torrent tracker row actions and a safe console dock; `deployment.changed` is an evidence-free SSE trigger for REST snapshot recovery. Torrent extend now requires bounded `additionalMinutes`, while release supports a row or confirmed all-waiting action under the existing `runtime-control` lock.
- Alpha 11 fixes the Web-only acceptance race found during alpha.10 installed v1 migration. MSI now requires the Agent service and protected named pipe in addition to Web health, fails immediately if Agent stops during an imported runtime rebuild, and allows up to 15 minutes for a slow but progressing WinPE rebuild. This keeps migration secrets action-scoped and prevents commit of a Web-only half-cutover. MSI `ProductVersion` advances to `2.0.26`; alpha.10 remains an unpublished migration-failed candidate.
- Alpha 10 fixes the failed-upgrade rollback defect found during alpha.9 installed acceptance: WiX now schedules removal of the previous product after `InstallInitialize`, inside the Windows Installer rollback transaction. A failed health probe must restore the prior binaries and services instead of leaving only State. MSI `ProductVersion` advances to `2.0.25`; alpha.9 remains an unpublished failed candidate.
- Alpha 9 makes v1 cutover an MSI-owned transaction: the importer now uses an atomic, idempotent non-secret backup, excludes plaintext deployment secrets while preserving them in the source fingerprint, repairs a missing rerun report, and rejects mismatched existing backups. MSI detects the standard v1 App/State roots before service startup, schedules projection rollback/commit around import, and gives Agent up to five minutes to rebuild runtime before health acceptance. Agent stays unavailable while `runtime.rebuildRequired` is pending, materializes DPAPI secrets only for the rebuild, captures SQLite product state after success, and leaves the marker pending on failure. MSI `ProductVersion` advances to `2.0.24`.
- Alpha 8 fixes management HTTPS rollback discovered during the unpublished alpha.6/alpha.7 acceptance builds: child-process failures now always reach transaction cleanup, failed certificate provisioning restores Agent and Web in dependency order with bounded retries, pinned local probes bypass system proxy settings, returning to loopback removes stale exported TLS PFX/CER files, and major upgrades preserve existing service settings instead of rotating tokens or resetting the management endpoint. MSI `ProductVersion` advances to `2.0.23`; alpha.6 and alpha.7 were never published and must not replace an existing same-version MSI.
- Alpha 6 fixes installed diagnostics and evidence export discovered after the immutable alpha.5 publication: packaged hosts no longer require a global npm command, OSDCloud import warnings cannot corrupt the PowerShell probe JSON, manual full diagnostics select the newest Fleet run, and ZIP creation now archives enumerated source entries instead of passing a literal wildcard. MSI `ProductVersion` advances to `2.0.21`.
- Removed unmanaged target-host prerequisites from `Prepare runtime`. The installed product now uses its bundled Node 24 executable, does not require npm on the deployment host, and ships hash-pinned OSD 26.4.23.1 plus OSDCloud 26.4.17.1 modules with their GPL notices and SBOM entries. Registered the IPv4 contract format inside Agent IPC, prepended the packaged module root to every Agent child process, reconciles missing download metadata for bundled software after v1 migration without replacing user-defined values, and creates interface/subnet-scoped inbound rules for DHCP, TFTP, HTTP and torrent data-plane traffic. The DHCP receiver binds to `0.0.0.0` so Windows can receive initial broadcasts; a second socket bound to the selected PXE IP sends one limited-broadcast reply, preventing a multi-NIC host from using its management adapter while remaining compatible with pre-address UEFI clients. Server-mode replies provide Options 66/67 directly and omit Options 60/43 so firmware uses TFTP instead of an unavailable BINL service on UDP 4011; proxy mode keeps its PXE discovery options. Boot-config reads DPAPI-protected deployment secrets on demand in Agent memory instead of depending on a scrubbed plaintext sidecar; provider failures return a redacted 503. Starting deployment ingress now requires a completed preflight with zero blocking failures. The v2 OS image response contract includes runtime cache annotations, preventing installed cached images from failing response validation. Software Test now preserves singleton `steps` as an API array, emits the required null recovery field during active runs, waits for the SYSTEM scheduled task to actually start before polling it, retries transient progress-file sharing violations, and retains raw runner failures only in the local diagnostic. Full uninstall removes product-owned firewall rules, while major upgrades preserve them. Alpha 5 uses monotonically higher MSI `ProductVersion 2.0.20`, so Windows Installer performs a real major upgrade from all earlier test packages. The replacement test candidate is `v2.0.0-alpha.5`.

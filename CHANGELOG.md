...
- Extra Unattend auto-login requires an explicit bounded test-only profile (1–3). SetupComplete still AutoLogons on first boot so the post-logon finalizer can run; removing that for production handoff is unfinished. Acceptance fixtures/router tools and tickets are excluded from Release. Physical/human evidence remains pending; no Release/package/master push.
...
- Lab cache validation hashes only `cache.requiredPaths`. Extra records left in an old manifest (generated `boot.wim` / `boot.ipxe`) are ignored so a stale hash cannot force another restore.
- Lab cache refresh now passes HostTools State `osdcloud-console.json` into `Restore-DeploymentArtifacts.ps1`, so `pxeinstall` is reset from the same secrets file the PXE HTTP server uses. Generated `boot.wim` / `boot.ipxe` are no longer hashed as cache inputs (they change on every endpoint sync).
- Lab fleet wait now fails closed as soon as WinPE posts a terminal error in `latest.json` (for example `selected-os.json` / SMB map failure) instead of waiting the full deployment timeout. Start-OSDCloud-iPXE validates the SMB `Z:` map and a non-empty OS root before reading `selected-os.json`.
- AutoLab client firmware is now a 2×2: Secure Boot On/Off is independent of TPM On/Off. Restore applies `-SecureBoot` and `-Tpm` separately (`Enable-VMTPM` / `Disable-VMTPM`) after `Winception-Clean`, which does not keep TPM. Default fleet remains four Secure Boot + TPM On VMs plus iPXE with both off. `Mode All` adds corner rounds: Secure Boot On + TPM Off, and iPXE Secure Boot Off + TPM On. Guest evidence asserts the expected pair per round. Host `/api/boot-mode` is still only `secureboot` or `ipxe`. The Lab workflow runs the merge-source `Invoke-WinceptionLabRegression.ps1`, not the installed HostTools copy.
...
- WinPE `ConvertFrom-Base64Url` returns a preserved `byte[]`. PowerShell was unrolling the array, so `Buffer.BlockCopy` failed after a successful boot-session 201.
- Desktop-ready reporting keeps the in-memory boot-session token until after the status POST, reads `deployment-progress.json` with share-read, and sets `DisableCAD` so AutoLogon is not blocked at the Windows 11 logon screen.

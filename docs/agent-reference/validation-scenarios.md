...
- Software Test VM changes follow [software-test.md](software-test.md). Test invalid VM/checkpoint/state, active ingress/Fleet and concurrent mutation conflicts under lock, SYSTEM execution, reboot codes, abort during waits/install, crash recovery, fail-closed cleanup and redaction. Full PXE acceptance remains separate.

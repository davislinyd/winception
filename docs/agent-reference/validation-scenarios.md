...
- A push to master runs only on the dedicated hyperv/winception-lab runner. The guard must validate the Internal Winception-AutoLab switch, vEthernet adapter 192.168.177.1/24, DHCP pool 192.168.177.200-250, four Secure Boot Gen2 VMs (`winception-autolab-01..04`), one Secure Boot-off iPXE VM (`winception-autolab-ipxe-01`), fixed 4 GiB memory, powered-off state, and Winception-Clean checkpoints. After checkpoint restore, Secure Boot VMs must have TPM On and the `MicrosoftWindows` template before PXE start. Historical `winception-client-01..04` vSwitch VMs are not valid AutoLab targets.
...
- The acceptance matrix is four Secure Boot VMs started in one batch followed by one dedicated iPXE VM. Each round requires API Fleet status completed at windows-desktop-ready and PowerShell Direct evidence for desktop marker, Explorer, no OOBE, Windows version, profile ID, and install sequence result. Secure Boot rounds also require guest `Confirm-SecureBootUEFI` and TPM Present/Ready/Enabled/Activated plus host TPM On. iPXE additionally requires snponly.efi, boot.ipxe, wimboot, and the WinPE callback.

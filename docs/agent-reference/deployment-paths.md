...
- Use `tools\Restart-HyperVms.ps1` from elevated PowerShell after Console Endpoint Sync, Preflight, and Start services. Default targets are AutoLab `winception-autolab-01..04`. It turns each VM off, enforces at least 4 GB fixed startup memory, optionally restores `Winception-Clean` (`-RestoreCheckpoint`; reapplies Secure Boot/TPM because that snapshot drops Hyper-V TPM), selects Network as first boot, and starts it. `-NoStart` leaves the VM Off after restore. Do not target `winception-autolab-router`, the software-test VM, or historical `winception-client-01..04` for AutoLab PXE. Do not lower the memory floor or re-enable Dynamic Memory for concurrent WinPE image application; Hyper-V ballooning below this floor can cause `System.OutOfMemoryException` during module loading or DISM `Expand-WindowsImage`. Confirm live `bootMode` and `bootFile` agree after Endpoint Sync before PXE (`secureboot` → `bootmgfw.efi`).

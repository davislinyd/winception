...
- The unattended merge path is a separate acceptance lane from both the physical-laptop path and the historical VM path. It uses only the dedicated Winception-AutoLab Internal switch on 192.168.177.0/24, service 192.168.177.1, and five dedicated Gen2 VMs: `winception-autolab-01..04` (Secure Boot On) plus `winception-autolab-ipxe-01` (Secure Boot Off). These names must not reuse the historical `winception-client-01..04` vSwitch regression VMs.

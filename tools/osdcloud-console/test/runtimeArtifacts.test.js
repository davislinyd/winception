...
test('installed runtime preparation uses packaged Node and does not require npm on the target', () => {
  ...
  assert.match(packager, /\$MsiVersion = '2\.0\.16'/);
...
test('WinPE torrent download shows local progress and active peers through loopback-only aria2 RPC', () => {
  ...
  assert.match(script, /\$lastTorrentProgressAt = Get-Date/);
  assert.match(script, /completedLength progress for 180 seconds; falling back to SMB-direct apply/);
  ...
  assert.match(script, /TORRENT SEED WAIT ACTIVE/);
  assert.match(script, /\[E\] Extend seed time    \[Enter\] Continue to reboot now/);
  assert.match(script, /Send-Screenshot -Stage 'torrent-seed-wait'/);
  assert.match(script, /Stop-OSDCloudProgressReporter/);
  assert.match(script, /Torrent seed wait active: \{0\} remaining/);

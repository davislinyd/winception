...
test('release metadata is aligned to v1.0.3-1', () => {
  const packageJson = JSON.parse(fs.readFileSync(packagePath, 'utf8'));
  const packageLock = JSON.parse(fs.readFileSync(packageLockPath, 'utf8'));
  const changelog = fs.readFileSync(changelogPath, 'utf8');
  const manual = fs.readFileSync(manualPath, 'utf8');

  assert.equal(packageJson.version, '1.0.3-1');
  assert.equal(packageLock.version, '1.0.3-1');
  assert.equal(packageLock.packages[''].version, '1.0.3-1');
  assert.match(changelog, /## v1\.0\.3-1 — 2026-07-19/);
  assert.match(manual, /Operations Manual · v1\.0\.3-1/);
  assert.match(manual, /Product documentation for Web v1\.0\.3-1/);
});
...
test('web UI exposes dashboard view topology', () => {
  ...
  assert.match(html, /Software ID <span class="required-marker"[^>]*>\*<\/span><\/span><input id="software-add-id" name="softwareId"[^>]*pattern="\[a-z0-9\]\[a-z0-9-\]\{0,15\}"/);
  ...
  assert.match(html, /Installer payload <span class="required-marker"[^>]*>\*<\/span>/);
  assert.match(html, /id="software-add-file-browse"/);
  assert.match(html, /id="software-add-file-name"/);
  assert.match(html, /id="software-add-stepper"/);
  assert.match(html, /新增 Client Software/);
  assert.match(html, /id="software-add-mode-template"/);
  assert.match(html, /id="software-add-mode-raw"/);
  assert.match(html, /value="template"/);
  assert.match(html, /value="raw"/);
  assert.match(html, /Guided installer/);
  assert.match(html, /Custom PowerShell/);
  assert.match(html, /id="software-add-depends-on"/);
  assert.match(html, /class="checkbox-list"/);
  assert.match(html, /id="software-add-network-requirement"/);
  assert.match(html, /id="software-add-network-probe-host"/);
  assert.match(html, /安裝後檔案驗證（選填）/);
  assert.match(html, /留白時只信任 installer 回傳碼/);
  assert.match(html, /3010.*成功但建議重開機/);
  assert.match(html, /1641.*重開機後從下一步續跑/);
  assert.match(html, /MSI 預設 <code>0,1641,3010<\/code>；EXE 預設 <code>0<\/code>/);
  ...
  assert.match(html, /id="software-test-summary"/);
  ...
  assert.match(script, /softwareAddRawScript\.disabled = isTemplate/);
  assert.match(script, /softwareAddNetworkProbeHost\.disabled = !needsInternet/);
  ...
  assert.match(html, /Custom PowerShell <span class="required-marker"[^>]*>\*<\/span>/);
  assert.match(html, /id="software-add-raw-template"/);
  assert.match(html, /Windows PowerShell 5\.1 syntax 檢查，因為 client 實際也使用該 runtime/);
  assert.match(script, /input\[data-software-dependency\]:checked/);
  assert.match(script, /const networkRequirement = elements\.softwareAddNetworkRequirement\.value/);
  assert.match(script, /let selectedSoftwareInstallerFile = null/);
  assert.match(script, /function customPowerShellTemplate\(installerType\)/);
  assert.match(script, /replace this discovery with the exact filename so selection is deterministic/);
  assert.match(script, /Do not call Restart-Computer from this script/);
  assert.match(script, /WINCEPTION_REBOOT_PENDING/);
  ...
  assert.match(script, /state\.guidedConsoleAttentionAction = action;[\s\S]*state\.guidedConsoleAttentionShown = false;[\s\S]*openDialog\(elements\.initializationDialog\);/);
  ...
  assert.match(script, /state\.guidedConsoleAttentionAction = 'endpoint-sync';[\s\S]*state\.guidedConsoleAttentionShown = false;[\s\S]*openDialog\(elements\.initializationDialog\);/);
  ...
  assert.doesNotMatch(html, /id="summary-action"[\s\S]*Run preflight/);
  assert.doesNotMatch(html, /Preflight not run/);
  ...
  assert.match(script, /function showSoftwareAddedDialog\(software\)/);
  ...
  assert.match(script, /const installationFields = input\.scriptMode === 'template'/);
  assert.match(script, /showSoftwareAddedDialog\(addedSoftware\)/);
  assert.match(styles, /\[hidden\]\s*\{\s*display: none !important;/);
  assert.match(styles, /\.software-add-stepper/);
  assert.match(styles, /\.software-add-mode-card\.selected/);

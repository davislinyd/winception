...
import { allowedCustomScriptExtensions, arrayFrom, assertInside, cleanCustomScriptFileName, deploymentProfileOptions, inputError, maybeString, normalizeHumanCatalogId, normalizeId, normalizePositiveInteger, normalizeProfileName, readJson, writeJson } from './shared.js';
...
export async function createCustomScript(config = {}, input = {}, options = {}) {
  const profileOptions = deploymentProfileOptions(config, options);
  const id = normalizeHumanCatalogId(input.scriptId ?? input.id, 'Custom script id');
  ...
  const reservedIds = new Set(Array.from(reservedCustomScriptIds(profileOptions, scriptRows), (value) => String(value).toLowerCase()));
  if (reservedIds.has(id.toLowerCase())) {
    throw inputError(`Duplicate custom script id or source: ${id}`, 409);
  }
  const source = id;
  const sourcePath = assertInside(
    profileOptions.customScriptsSourceRoot,

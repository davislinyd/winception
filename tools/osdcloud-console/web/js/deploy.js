...
export function renderEndpointSummary(appState) {
...
export function renderOperation(appState) {
...
export function renderRuntimeReadiness(appState) {
...
export function serviceAddress(service) {
...
export function appendFleetLastSeenCell(row, value) {
  const cell = document.createElement('td');
...
export function customFilterValues(input) {
...
export function selectedOsCatalogFilters() {
  return {
    osFamily: ['win11'],
    edition: ['Pro'],
    activation: ['Retail'],
...
export function osCatalogFiltersReady(filters = selectedOsCatalogFilters()) {
  const missing = [];
...
export function catalogReadinessMessage(readiness) {
...
export function catalogFilterQuery(filters) {
...
export function clearRefineFilters() {
  [
...
export function formatOsFamily(value) {
  return osFamilyLabels.get(String(value ?? '').toLowerCase()) ?? text(value);
}

export function formatCatalogFilterSummary(filters = state.osDownloadCatalogFilters) {
  if (!filters) {
    return '';
  }
  const parts = [
...
export function renderOsImageSummary(appState) {
...
export function selectOptions(select, values, emptyLabel) {
...
export function renderOsCatalogFilters() {
...
export function filteredOsDownloadCatalog() {
...
export function renderOsImportStatus(appState) {
...
export function importMetadataFromInputs(suggested = {}) {
  return {
    ...suggested,
...
export function fillImportMetadataDefaults(suggested = {}) {
...
export function renderOsImportIndexes(appState) {
...
export function renderOsImages(appState) {
  const osState = appState.osImage;
...
export function showValidationEvidence(runId) {
  if (!runId) {
    return;
  }
...
export function renderClients(appState) {
...
export function renderChecks(element, checks, emptyText = 'No data observed yet.') {
...
export function preflightResolutionHint(check) {
...
export function preflightTooltip(check, detailText, statusLabel) {
...
export function preflightStatus(checks) {
  if (!checks?.length) {
...
export function issueRank(check) {
...
export function renderDriverPackCache(appState) {
...
export function isActiveInterface(item, config) {
...
export function isPendingInterface(item) {
...
export function currentInterfaceChoice() {
...
export function appendInterfaceStatusRow(message, className = '') {
  const tr = document.createElement('tr');
  if (className) {
...
export function renderInterfaces(appState) {
...
export function renderSoftwareCatalog(appState) {
...
export function renderScriptCatalog(appState) {
...
export function payloadChecks(appState) {
...
export function renderPayload(appState) {
  const active = appState.profile?.activeProfile;
...
export function endpointStatusText(appState) {
  return (appState.endpointUpdateStatus ?? []).join('\n').toLowerCase();
}

export function syncStepState(appState, doneKeywords, workingKeywords) {
...
export function renderStepList(element, appState) {
  element.replaceChildren();
...
export function activeBootMode(config) {
  return config.dhcp?.bootMode === 'ipxe' ? 'ipxe' : 'secureboot';
}

export async function handleBootModeChange(mode) {
...
export function renderBootMode(appState) {
...
export function runEvents(appState) {
...
export function evidenceKeyMatches(keyNorm, needle) {
  return keyNorm === needle || keyNorm.endsWith(needle);
}

export function findEvidenceEntry(entries, needle, matchMode) {
...
export function evidenceText(value, options = {}) {
...
export function evidenceValue(appState, keys, options = {}) {
...
export function latestRunEvent(appState, predicate) {
  return [...runEvents(appState)].reverse().find(predicate) ?? null;
}

export function makePreformattedValue(value) {
...
export function validationCheck(appState, matcher) {
  return (appState.validation ?? []).find((check) => matcher.test(check.name ?? ''));
}

export function renderValidation(appState) {
...
export function renderTimeline(appState) {
...
export function isScrolledToBottom(element, tolerance = 2) {
  return element.scrollHeight - element.scrollTop - element.clientHeight <= tolerance;
}

export function renderLogs(appState) {
...
export function renderWarningBanner(appState) {
...
export function renderDashboardTiles(appState) {
...
export function setStrip(el, text, tone) {
  if (!el) return;
...
export function renderStatusStrip(appState) {
  const runtimeReady = appState.runtime?.ready === true;
...
export function renderLiveMetrics(appState) {
...
export function renderPipeline(appState) {
...
export function topologyRingClass(run) {
  if (run.status === 'completed') return 'ring done';
  if (run.status === 'failed') return 'ring';
  return 'ring';
}

export function renderTopology(appState) {

...
Nested deployment 測試期間，外層 Hyper-V 主機不得睡眠或進入 Modern Standby。Winception host VM 內的 timer 會在 VM 暫停時安全暫停，但 guest 無法阻止外層主機睡眠；該輪不屬於不中斷驗收。

開始前記錄外層主機目前的 AC sleep timeout，並在測試窗口暫時設為 **Never**。完成後恢復原值；Winception 不會自動修改外層主機的 power plan。

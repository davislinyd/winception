...
| Expanded AutoLab | Failed / cleanup Passed | `20260915e` completed Router PXE, Fleet `windows-desktop-ready` and PowerShell Direct with the matching test profile, Secure Boot and TPM, then failed at guest `New-NetNat`: `Router New-NetNat failed: Invalid class`. Original profile/endpoint restored, services stopped, Fleet cleared, six VMs Off, firmware/Clean checkpoints restored and 29-check Preflight passed. Router Ready and the full network matrix remain blocked. |

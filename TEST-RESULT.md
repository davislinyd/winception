...
| --- | --- | --- |
| Source | Passed | acceptance:source: check, 483 tests / 480 passed / 3 skipped, smoke; logs .ai/acceptance-source-network-review.log (codefc48f36) and ignored JSON/HTML test-results/acceptance-source |
...
| Expanded AutoLab | Failed / cleanup Pending | Owned router Create/Clean checkpoint succeeded after localized NIC fix. Initial Bootstrap stopped before mutation on relative config path; Resolve-Path fix and 24 focused tests passed; Bootstrap ValidateOnly Passed. Router PXE obtained192.168.177.200, but boot-session403 on stale pre-restore dynamic MAC scope; no Fleet/credentials/install. Services stopped and router Off; at23:38 elevated snapshot confirmed old runner PID16072 waits timeout then cleanup; AI quota handoff, cleanup still Pending. Source fixed and26 focused tests passed; not rerun. Full network matrix not executed. Additional source review27/27 covers shared post-restore scope, port-qualified Proxy denial/no-issued-credentials and retained bootstrap round evidence |

...
| --- | --- | --- |
| Source | Passed | Latest `npm run acceptance:source`: check, 486 tests / 483 passed / 3 skipped, smoke; Router restart recovery is committed at `1ba36eb` |
...
| Expanded AutoLab | Failed / cleanup Passed | `20260915f` reached Fleet `windows-desktop-ready` and enabled Router nested virtualization, then guest Hyper-V servicing left the VM Off and PowerShell Direct timed out. The original profile, endpoint, processor state, services and six VMs were restored; elevated post-cleanup ValidateOnly passed. Restart recovery is committed at `1ba36eb` but not live-proven; Router Ready and the network matrix remain blocked. |
...
2026-09-15 19:34–20:08 +08，以 `1ba36eb` 執行新的唯一一次 `acceptance-router-bootstrap-20260915g`。前置 ValidateOnly 通過；Fleet run `20260915-193921-3833-6458-5439-4386-0617-9856-93` 與 PowerShell Direct 均到達 `windows-desktop-ready`，profile `YZ6XQK8R`、Windows 11 build 26200、Secure Boot On／`MicrosoftWindows`、TPM On、四個安裝步驟均有當次證據。Router guest setup 隨後停機並設定 2 vCPU／nested On；因部署輪保留 Network-first，重新啟動後再次進入 PXE 而無法取得 PowerShell Direct，逾時後 `status=Failed`、`cleanup=Passed`。未建立 Router Ready，未執行網路矩陣。證據位於 `C:\OSDCloud\HostTools\State\lab\evidence\acceptance-router-bootstrap-20260915g`；cleanup 後服務停止、profile／endpoint／processor 與六台 VM 全部還原。Source 已改為 guest setup 前驗證並切換 owned Router 的 FirstBootDevice 至部署硬碟；30 個相關 tests、486 tests（483 passed／3 skipped）與 smoke 通過，修正尚未 live-proven。

...
The signed-MSI lifecycle gate is complete on alpha.4, and exact alpha.5 MSI upgrade, one Generation 2 Secure Boot PXE client and one isolated Software Test checkpoint restore are complete. This closes the internal prerelease deployment gate. Final production acceptance still requires installed v1 migration, failed-health-probe rollback, two consecutive four-VM rounds, physical-laptop PXE, multi-client torrent offload, Offline ISO, diagnostics/evidence export and HTTPS LAN opt-in. Current self-signed certificates are for approved internal test hosts; public/organization certificates are a later release input. Skipped tests are not accepted.

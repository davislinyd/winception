...
`v2.0.0-alpha.1` is superseded for installation acceptance because a clean host rejected the expected untrusted self-signed root before explicit trust. Do not replace its immutable assets; use `v2.0.0-alpha.2`.

The internal prerelease gate requires elevated signed-MSI fresh install/repair/uninstall/reinstall with State persistence, one Generation 2 Secure Boot client and one isolated Software Test VM restore on the exact alpha.2 candidate. Final production acceptance additionally requires installed v1 migration, upgrade rollback, two consecutive four-VM rounds, physical-laptop PXE, torrent, Offline ISO, diagnostics/evidence export and HTTPS LAN opt-in. Current self-signed certificates are for approved internal test hosts; public/organization certificates are a later release input. Skipped tests are not accepted.

## v2 alpha.2 local candidate evidence — 2026-07-14
...
- Package: exact alpha.2 MSI and bootstrap hashes are recorded in the matrix. Extraction matched 6,842/6,842 payload records and 68/68 applicable signatures. The packaged Agent/Web/API/manual/SQLite smoke passed from extracted files without installing services.

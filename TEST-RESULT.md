...
- Local MSI evidence: the package build completed its stage/WiX work but the wrapper timed out before output signing; the same complete stage was then continued through signing and manifest generation. MSI `ProductVersion 2.0.31`, SHA-256 `C97CE84F02A8604D1A583862DCFAD8777458A744E0A3CAF9892762DE13075844`, and the bootstrap have valid expected self-signed signatures; local bootstrap `Check` passed 20/20. This remains uninstalled-package evidence only: OSD Server service/health verification, PXE and physical-laptop acceptance remain unrecorded. Remote CI, prerelease publication, Pages publication, tag and `master` merge were not run.
...
|---|---|---|---|---|---|
| `2.0.0-alpha.14` / package source `ae7849a` | Local alpha.14 gates passed: v1 418/418 and v2 48/48 with 0 skipped; typecheck, lint, zero-cycle, docs/diagram/OpenAPI drift, PowerShell parse, MSI command/signature, smoke and bilingual documentation checks passed. Alpha.12 was rejected after live WinPE proved its UTC clock could be offset; alpha.13 replaced wall time with a monotonic timer but was rejected after a second-round progress-file sharing violation. Alpha.14 adds bounded atomic replacement retries. Remote product CI was intentionally not run | MSI `ProductVersion 2.0.29`, SHA-256 `267DF3BF3B3342EE62BFA4B874B76930979FC0EA52402F9F3EE2EA9D9E99E3DA`; manifest matches source `ae7849ab606b4c8ae3fc61184dd305f1d19650c4`, Authenticode is valid with expected self-signed thumbprint `80A4429C308A3BFF0A12107DA8F1C5AB21DE4733`, and installed bootstrap passed 34/34. `OSD Server` health reported `2.0.0-alpha.14` | Preflight passed 29/29 before both clean rounds. Round 1 runs `20260715-212821-3165-2914-1943-0908-5094-0852-36` and `20260715-212821-0885-8703-1155-6903-2654-8648-29` reached `windows-desktop-ready` in 1,510/1,524 seconds with 74/79 events. Round 2 runs `20260715-215639-3165-2914-1943-0908-5094-0852-36` and `20260715-215639-0885-8703-1155-6903-2654-8648-29` reached the same state in 1,765/1,682 seconds with 95/86 events. All four runs had 0 warnings, `seedReason=deadline`, peer count 2, no fallback, about 3.183/3.185 GB uploaded per client and 0 progress sharing violations. The user-approved resource-limited scope is two consecutive two-client rounds because four clients saturated the 32 GB outer host. An outer-host Modern Standby interruption was diagnosed separately and is not counted | Not run | Exact alpha.14 two-round multi-client VM/torrent gate accepted for the approved two-client resource scope. Internal and unpublished; physical-laptop and remaining external release rows stay open |
| `2.0.0-alpha.11` / package source `a517d2e` | Full local gate passed on source `153025c`: complete v1 suite including all 3 aria2 integrations with 0 skipped; v2 48/48 at 94.00% line/85.06% branch and critical 99.01%/92.21%; typecheck, lint, zero-cycle, docs/diagram/OpenAPI drift, 40-file PowerShell parse, MSI command/signature, Web Playwright 3/3, docs Playwright 2/2, bilingual Pages/offline builds, production audit 0, secret rules and Gitleaks. Packaged production inputs are unchanged from `a517d2e`; the later source commits contain evidence and a test-only release-metadata assertion. Remote product CI was intentionally not run | MSI `ProductVersion 2.0.26`, SHA-256 `A369CE6FF6E3D2DE752BC0EA78B853E6B63561F2EDF44A0AE445D501F3A427D2`, manifest match and valid expected self-signed Authenticode. Exact packaged dry-run read 14 settings, 4 profiles, 3 software entries, 1 script and 3 secrets without backup/payload writes or disclosure. A first no-network install intentionally lacked aria2; Agent stopped, the new probe forced MSI 1603 rollback, product/services were removed, v1 remained and action plaintext was scrubbed. After staging catalog-exact aria2 SHA-256 `BE2099C214F63A3CB4954B09A0BECD6E2E34660B886D4C898D260FEBFE9D70C2`, the same retained source resumed to install 34/34 and repair 34/34. DB counts matched the import; secrets were DPAPI ciphertext, runtime rebuild flag was false, backup excluded plaintext secrets, source remained ACL-protected, no transaction/secret residue remained, Agent/Web accounts, pipe/DACL, profile read, health and `/manual/` passed. A further packaged CLI rerun returned `already-imported` without disclosure | Installed migration/rollback guest used a clean checkpoint of the no-network Generation 2 VM, fixed 4 GiB and Secure Boot ON. Ready boot artifacts were staged from the exact validated host runtime; no PXE/deployment parity run was claimed for alpha.11 | Not run | Installed v1 dry-run, failure rollback, resumable import, runtime rebuild, repair, idempotent rerun and full local release suite accepted. Internal and unpublished; multi-client/physical release rows remain open |
| `2.0.0-alpha.10` / package source `31efcac` | Local source/package gates passed: relevant v1 Web UI 14/14 and v2 48/48; typecheck, lint, zero-cycle, docs/diagram/OpenAPI drift, 40-file PowerShell parse, MSI command/signature checks, bilingual offline docs build and production audit 0. Remote product CI was intentionally not run | MSI `ProductVersion 2.0.25`, SHA-256 `781A065357822E5582BA12DDDC1241F778FA0007FEFA70E74F77B3FD16A80D2A`, manifest match and valid expected self-signed Authenticode. With alpha.8 stopped and an HTTP 503 blocker owning 127.0.0.1:8080, the 300-attempt health probe ended with expected status 1603. Transactional major-upgrade rollback restored both services, exact prior package-manifest/SQLite/service-settings hashes, legacy config count and Windows Installer registration `2.0.23`; after blocker removal Agent/Web ran and health reported `2.0.0-alpha.8` | Exact packaged dry-run imported 14 settings, 4 profiles, 3 software entries, 1 script and 3 protected secrets without backup/payload writes or secret disclosure. Installed migration then exposed a Web-only acceptance race: MSI committed while Agent was still rebuilding runtime, bootstrap failed `agent-named-pipe`, and a later DISM save error stopped Agent with `runtime.rebuildRequired=true`. Action-lifetime plaintext was scrubbed and the v1 source remained. Alpha.11 must require Agent pipe readiness before commit | Not run | Failed-upgrade rollback accepted, but installed migration rejected. Unpublished and superseded by alpha.11 |
...
`v2.0.0-alpha.1` is superseded because it rejected the expected untrusted self-signed root. `v2.0.0-alpha.2` is superseded because a fresh Windows host could lack the machine-level TrustedPublisher registry store key. `v2.0.0-alpha.3` is superseded because its WiX provisioning argument was invalid on a fresh install. `v2.0.0-alpha.4` is superseded by the installed-runtime-complete alpha.5 release. Alpha.12 and alpha.13 are rejected and unpublished; alpha.14 is the current validated internal candidate. Do not replace immutable releases; the latest published prerelease remains `v2.0.0-alpha.5` until a new tag is created.

The signed-MSI lifecycle gate is complete on alpha.4, and exact alpha.5 MSI upgrade, one Generation 2 Secure Boot PXE client and one isolated Software Test checkpoint restore are complete. Alpha.8 additionally closes installed major-upgrade settings preservation, HTTPS LAN opt-in/fail-closed/loopback restoration, and Offline ISO host plus no-network guest acceptance. Alpha.9 failed the installed health-probe rollback gate; alpha.10 closed transactional major-upgrade rollback but failed Agent-aware migration acceptance; alpha.11 closes installed v1 dry-run, failure rollback, resumable import, runtime rebuild, repair, idempotent rerun and the complete local release suite. Alpha.14 closes the approved resource-limited two-round, two-client Secure Boot PXE and torrent-offload gate. Physical-laptop PXE remains open. Current self-signed certificates are for approved internal test hosts; public/organization certificates are a later release input. Skipped tests are not accepted.

## v2 alpha.14 two-round multi-client evidence — 2026-07-15

- Exact package: source `ae7849ab606b4c8ae3fc61184dd305f1d19650c4`, MSI `ProductVersion 2.0.29`, SHA-256 `267DF3BF3B3342EE62BFA4B874B76930979FC0EA52402F9F3EE2EA9D9E99E3DA`, valid expected self-signed Authenticode signature. Installed health reported `2.0.0-alpha.14`; endpoint sync completed and preflight passed 29/29.
- Candidate history: alpha.12's UTC deadline failed live validation because WinPE UTC was offset by 16 hours. Alpha.13's cross-process monotonic timer passed both live rounds, but its second round exposed a `File.Replace` sharing violation that lost one final desktop-ready update. Alpha.14 retries transient atomic replacement for up to four seconds; deterministic lock injection and the local source suite passed before packaging.
- Resource scope: an earlier four-client round completed once, but a second four-client round saturated the 32 GB outer Hyper-V host. The user approved reducing the formal scope to two clients. Both clients used Generation 2, fixed 4 GiB, Secure Boot ON, EFI network first and the same clean `Before alpha.5 PXE validation` checkpoint before each round.
- Clean round 1: the two `212821` runs listed in the matrix completed at `windows-desktop-ready`, 100%, with 0 warnings and no sharing violation. Torrent peer count was 2, fallback remained false, seed release was the natural deadline, and uploaded bytes were `3183476736` / `3185004694`.
- Clean round 2: the two `215639` runs listed in the matrix completed at `windows-desktop-ready`, 100%, with 0 warnings and no sharing violation. Torrent peer count was 2, fallback remained false, seed release was the natural deadline, and uploaded bytes were again `3183476736` / `3185004694`.
- Harness interruption: one preliminary alpha.14 round crossed an outer-host five-minute AC sleep timeout. Kernel-Power recorded Modern Standby; client monotonic time paused and safely resumed, but the round ended 1/2 desktop-ready and is not counted. A temporary outer-host system-required request prevented sleep during the two clean rounds and was removed afterward. A nested Winception guest cannot prevent its outer Hyper-V host from sleeping.
- Cleanup: DHCP/TFTP/HTTP/Torrent were stopped through `/api/v2`, both clients were restored and left Off, final preflight passed 29/29, `OSD Server` was restored to fixed 12 GiB and health remained alpha.14 with deployment ingress stopped.

...
The internal prerelease gate requires elevated signed-MSI fresh install/repair/uninstall/reinstall with State persistence, one Generation 2 Secure Boot client and one isolated Software Test VM restore on the exact candidate. Final production acceptance additionally requires installed v1 migration, upgrade rollback, two consecutive four-VM rounds, physical-laptop PXE, torrent, Offline ISO, diagnostics/evidence export and HTTPS LAN opt-in. Current self-signed certificates are for approved internal test hosts; public/organization certificates are a later release input. Skipped tests are not accepted.

## v2 local candidate evidence — 2026-07-14

- Source: v1 405/405, v2 41/41, Web E2E 3/3, Docs E2E 2/2, zero production cycles, 36 PowerShell files parsed and no remote product CI run.
...
- Package: exact MSI and bootstrap hashes are recorded in the matrix. Extraction matched 6,842/6,842 payload records and 68/68 applicable signatures. The packaged Agent/Web/API/manual/SQLite smoke passed from extracted files without installing services.
- Not evidence: no elevated per-machine install, repair, uninstall, State-persistence lifecycle, PXE client or Software Test was executed in this shell. Therefore no prerelease, Pages publication or production-ready claim is authorized by this record.

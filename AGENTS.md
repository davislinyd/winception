...
- Deployment profile changes must test catalog/profile validation, profile add/edit/delete validation, safe publish roots, selected-only ordered payload publishing, empty profiles, `Install-Apps.ps1` selected-only / ordered / missing-selected-app behavior, and that publishing a profile also republishes its `osImage` to `selected-os.json` / `smb.imagePath`.

...
- Optional network matrix adds only owned Gen2 winception-autolab-router (fixed 4 GiB, Secure Boot/TPM On). LAN is Internal AutoLab/.254; Default Switch is WAN upstream only. Explicit bootstrap uses two vCPUs, enables nested virtualization only while the owned VM is Off, sets its deployed hard disk first before guest setup, enables guest Hyper-V with a controlled restart, and may start only that owned VM when feature servicing leaves it Off. Require `MSFT_NetNat` before creating WinNAT or Ready. On failure restore the prior processor settings and collect guest NAT diagnostics before VM cleanup. Verify ownership/VHD parent chain/tool hash/clean+ready checkpoints. Missing prerequisites block master. Independent DHCP .100–.149 has no PXE options and must stop before Winception Server .200–.250.
- Physical/AutoLab share Global\Winception-AutoLab. Physical defaults readonly; require ignored onsite config, disposable MAC/UUID, Execute and confirmed DHCP-free window for self-managed DHCP. Foreign ICS/NAT is Blocked, never repaired. SetupComplete still AutoLogons on first boot (`AutoLogonCount` 5) so the post-logon finalizer can run; terminal state then clears AutoLogon. Extra Unattend auto-login is test-only (`acceptance.testOnly`, count 1–3). Removing first-boot AutoLogon for production handoff is unfinished (`docs/debugging-todo.md`). Cleanup removes test tasks/tickets and restores original profile/endpoint; missing post-stop report or cleanup failure prohibits complete pass.

- `.github/workflows/pr.yml` is source-only: it may run Node `node --check`, PowerShell parser, `npm run acceptance:source`, targeted Node tests (`webUi`, `beginner`, `labAutomation`), and `npm run acceptance:ui` in the checkout workspace, but must not write `C:\OSDCloud`, run Endpoint Sync, publish profiles, start services, or mutate DHCP. It does not run full `npm test`, `npm run check`, or `npm run smoke`.
...
- `.github/workflows/publish-pages.yml` is documentation-only `workflow_dispatch` on `ubuntu-latest`. It builds `docs/winception-operations-manual.html` through `tools/Build-GitHubPages.ps1` (root `index.html`) and also copies `docs/winception_torrent_deck/` to `torrent/`. It must not run on `master` push, create a tag/Release, or write `C:\OSDCloud`.
...
- Treat `Initialize-WinceptionLab.ps1 -ValidateOnly` as the read-only prerequisite check. The non-validate bootstrap is a one-time operation and must verify the exact adapter, five powered-off Gen2 VMs, Winception-Clean checkpoints, default Secure Boot/TPM roles, and no foreign DHCP binding before changes. `Winception-Clean` does not keep Hyper-V TPM; restore must apply `-SecureBoot` and `-Tpm` independently. `Mode All` also runs firmware corners: `01` Secure Boot On + TPM Off, and the iPXE VM Secure Boot Off + TPM On.
...
- Stop all target VMs before restoring any checkpoint. Restore failures must not skip later VMs. Wait for a stable current-adapter Network firmware source, preserve an already-first entry, and verify the restored Secure Boot/TPM role. After the round static MAC is assigned, re-apply that Network source as first boot so the firmware path binds the new MAC. Inspect BootOrder property existence before reading `BootType` or `Device.Id`; skip transient/malformed entries instead of throwing under StrictMode. Never loosen State ACLs to bypass a Lab failure. Guest Windows 11 acceptance uses `CurrentBuild` >= 22000; registry `ProductName` may still say Windows 10 Pro.

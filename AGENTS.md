...
- When updating setup or deployment docs, keep the README `新主機 Clone 後啟動流程` current so another operator can clone, run lightweight setup, start Web, prepare runtime from Runtime Readiness, select/sync the service endpoint, run preflight, manually start services, and validate completion without reading prior chats.

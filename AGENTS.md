...
- `.github/workflows/publish-pages.yml` is documentation-only `workflow_dispatch` on `ubuntu-latest`. It builds `docs/winception-operations-manual.html` through `tools/Build-GitHubPages.ps1` and must not run on `master` push, create a tag/Release, or write `C:\OSDCloud`.

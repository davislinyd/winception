...
- Client app payloads belong under `C:\OSDCloud\Win11-iPXE-Lab\Media\OSDCloud\Apps`. WinPE shutdown copies them into deployed Windows at `C:\ProgramData\OSDCloud\Apps`; `SetupComplete` then runs `Install-Apps.ps1` silently before the desktop-ready marker. Current payload installs `7zip\7z2601-x64.msi` with `msiexec /qn /norestart REBOOT=ReallySuppress`.

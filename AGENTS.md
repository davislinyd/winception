...
- TUI v0.2.10 adds optional Google Chrome Enterprise client software through the `default-chrome` deployment profile. Keep the normal `default` profile on 7-Zip only unless the user deliberately selects the Chrome profile.
...
- TUI v0.2.13 deployment profile creation generates an 8-digit numeric profile id server-side and must avoid collisions with existing profile ids and JSON file names. Operators edit the profile name as display text; profile id remains the stable service key.

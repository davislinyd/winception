...
- Deployment profile changes must test catalog/profile validation, safe publish roots, selected-only payload publishing, empty profiles, and `Install-Apps.ps1` selected-only / missing-selected-app behavior.

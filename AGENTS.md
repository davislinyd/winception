...
- AutoLab VMs are `winception-autolab-01..04` (Secure Boot On, TPM On, `MicrosoftWindows` template) and `winception-autolab-ipxe-01` (Secure Boot Off). Do not reuse historical `winception-client-01..04` vSwitch regression VMs.
- Treat `Initialize-WinceptionLab.ps1 -ValidateOnly` as the read-only prerequisite check. The non-validate bootstrap is a one-time operation and must verify the exact adapter, five powered-off Gen2 VMs, Winception-Clean checkpoints, Secure Boot roles, TPM on Secure Boot VMs, and no foreign DHCP binding before changes. `Winception-Clean` does not keep Hyper-V TPM; `Initialize-WinceptionLab.ps1` and `Restore-LabCheckpoint` must re-enable TPM on Secure Boot VMs after firmware apply. Do not enable TPM on the iPXE VM.

...
- Live deployment still runs from `C:\OSDCloud`; a Git clone alone is not a deployable PXE runtime until the repo-only bootstrap downloads or rebuilds the cataloged runtime artifacts.
...
- Preferred new-host bootstrap is repo-only rebuild: clone the clean GitHub repo, create untracked local secrets, then run `Deploy-DeploymentServer.cmd` or `tools\Initialize-DeploymentServer.ps1` without `-ArtifactBundle`. The bootstrap may auto-install missing Windows ADK Deployment Tools / Windows PE Add-on from Microsoft-signed installers, create a missing `%ProgramData%\OSDCloud` default template from ADK WinPE media, tolerate a broken PowerShellGet / `Save-Module` path by skipping WinPE PowerShell Gallery module injection, use a .NET SHA256 fallback when `Get-FileHash` is unavailable, download/rebuild `C:\OSDCloud`, verify artifact hashes, create/update the local `pxeinstall` read-only SMB account and `OSDCloudiPXE` share, sync endpoint, run preflight, and start the Web console, but it must not auto-start DHCP/TFTP/HTTP deployment services. `-ArtifactBundle` is legacy fallback only for offline or bit-for-bit restore.
- Bootstrap must not silently change Windows NIC IP settings unless `-ConfigureNic` is explicit. If preflight fails with `Service IP <ip> not assigned to any IPv4 interface` or `EADDRNOTAVAIL`, tell the operator to identify the PXE/client NIC with `Get-NetAdapter` and rerun `tools\Initialize-DeploymentServer.ps1 -InterfaceAlias '<PXE-NIC-NAME>' -ServerIp '<service-ip>' -PrefixLength <prefix> -ConfigureNic`, or manually assign the IP before rerunning preflight.
...
- When updating setup or deployment docs, keep the README `新主機 Clone 後啟動流程` current so another operator can clone, rebuild `C:\OSDCloud`, start `npm run web`, select the service endpoint, run preflight, start services, and validate completion without reading prior chats.

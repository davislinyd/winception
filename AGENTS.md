...
- DHCP reservations may pin known physical client MAC addresses to fixed IPs, such as `AA-BB-CC-DD-EE-FF` -> `192.168.100.115`. This is only a duplicate-DHCP mitigation; it does not prove the client selected the host DHCP responder or remove the need to isolate DHCP for final validation.

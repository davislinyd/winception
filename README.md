...
目前穩定版為 `v0.6.7`，永久保留於 tag `v0.6.7` 與 `release/v1`。v2 在 `codex/v2-familiar-ux-alpha15` 開發；最新公開測試版為 `2.0.0-alpha.5`，目前 source candidate 為 `2.0.0-alpha.16`。完成 MSI、migration、VM、實機與 release-readiness matrix 前不可取代 `master`。
...
- v2 Web 採用 v1 熟悉的 Deploy／Monitor、service cards、Fleet、torrent tracker 與 console dock；畫面只使用 `/api/v2` read model。`deployment.changed` SSE 只觸發 REST snapshot recovery，不傳 Fleet、log 或 evidence 內容。
...
The stable line is `v0.6.7`, preserved by tag `v0.6.7` and branch `release/v1`. v2 is developed on `codex/v2-familiar-ux-alpha15`; the latest published test build is `2.0.0-alpha.5`, while the current source candidate is `2.0.0-alpha.16`. It must not replace `master` until signed MSI, migration, VM, physical-laptop and release-readiness acceptance are complete.
...
The local alpha product core is implemented: SQLite is authoritative for structured configuration and catalogs, legacy JSON is a controlled compatibility projection, the service-SID pipe DACL is applied and read back after Agent listen, and production evidence roots use retention/index maintenance. The v2 Web presents the familiar v1 Deploy/Monitor workflow through v2 read models; `deployment.changed` SSE is only a REST snapshot-recovery trigger and carries no Fleet, log or evidence data. Existing OSDCloud behavior runs through a coordinated compatibility adapter; every v2 mutation remains inside schema and resource-lock boundaries.

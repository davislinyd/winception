...
目前穩定版為 `v0.6.7`，永久保留於 tag `v0.6.7` 與 `release/v1`。v2 在 `codex/v2-rewrite` 開發；最新公開測試版為 `2.0.0-alpha.5`，目前 source candidate 為 `2.0.0-alpha.14`。完成 MSI、migration、VM、實機與 release-readiness matrix 前不可取代 `master`。

> **v2 暫停維護 / Temporarily unmaintained:** v2 目前不發佈後續 alpha、修正或支援；既有 alpha 僅保留作內部歷史測試，且不可作為升級路徑。 v2 will not receive further alpha releases, fixes, or support at this time; existing alpha builds are retained only for internal historical testing and are not an upgrade path.
...
The stable line is `v0.6.7`, preserved by tag `v0.6.7` and branch `release/v1`. v2 is developed on `codex/v2-rewrite`; the latest published test build is `2.0.0-alpha.5`, while the current source candidate is `2.0.0-alpha.14`. It must not replace `master` until signed MSI, migration, VM, physical-laptop and release-readiness acceptance are complete.

...
- **技術**：WinPE 端用單一靜態執行檔 `aria2c.exe`（開源，邊下載邊做種）當 BT client；host 端在既有 Node console 內跑 `bittorrent-tracker`（MIT）當 tracker，並用同一支 `aria2c.exe` 對 OS `.torrent` **做種（seeder）**當 swarm 來源。
...
- **種子節流（關鍵）**：host 種子預設以 `--max-upload-limit`（`seederMaxUploadLimit`，預設 `50M`）限速。單一快速種子若不限速，會在 peer 還來不及互傳前就把每台餵滿（實測 4 台 → host 上傳 4 份）。限速讓種子成為瓶頸，逼 client 之間互相交換 piece，host 只需上傳約 1 份。
- **host 服務**：啟動服務後會多一個 `Torrent Tracker`（tracker 預設 `serverIp:6969`、種子 listen `6881`），隨 `Start all services` 一起啟停；Dashboard 卡片顯示 `… · seeding <檔名>`。
- **種子 log**：host 種子輸出寫到 `C:\OSDCloud\logs\torrent-seeder.log`，含週期上傳摘要（`UL` 速度 + 累積上傳量 + seed ratio）與連線數 `CN`。預設 level `notice`（避免逐 block 灌爆）；要看 per-peer piece 細節可設 `seederLogLevel: "info"`（log 會很大）。
...
- **runtime 需求**：`aria2c.exe` 由 `Prepare runtime` 依 `config\runtime-artifacts.json` 下載（zip 解壓 + SHA-256 驗證，不進 Git），endpoint sync 時注入 `boot.wim`。`.torrent` 與其 sidecar `os-torrent.json` 會在 profile publish / endpoint sync 時於 OS cache 內重新產生（announce URL 內嵌 service IP），種子也會跟著重啟。
...
> 注意：P2P 落地路徑會在套用前重新分割目標磁碟，屬於部署關鍵路徑變更。請先以 VM regression 路徑驗證再用於大量實體部署；SMB 直接套用回退會完整保留。`seederMaxUploadLimit` 太高（接近 line rate）會讓 offload 失效、太低則拖慢首份散佈，依網路頻寬調整。
...
4. 發佈後 `selected-profile.json` 會寫出 `execution.defaultTimeoutSeconds` 與 `installSequence: [{type, id, timeoutSeconds?}]`，並保留 `selectedSoftware` / `software` 供 client 端與診斷摘要使用。WinPE shutdown 會把 `Scripts` 子樹一併複製到 client 端 `C:\ProgramData\OSDCloud\Scripts`，SetupComplete 透過 `Install-Apps.ps1` 照 `installSequence` 執行 `Apps\<softwareId>\install.ps1` 或 `Scripts\<scriptId>\run.ps1`。
...
8. `Install-Apps.ps1` 會在 client 端保留整體 transcript `C:\Windows\Temp\osdcloud-logs\apps-install.log`，並為每個 step 建立 `C:\Windows\Temp\osdcloud-logs\install-sequence\<sequence>-<type>-<id>-<timestamp>.log`、整體 `C:\Windows\Temp\osdcloud-logs\install-sequence-summary.json`、shared state `C:\Windows\Temp\osdcloud-logs\install-sequence-state.json`，以及 custom script 視角的 `C:\Windows\Temp\osdcloud-logs\custom-scripts-summary.json`。Runner 會對每一步注入 `OSDCloudInstallStatePath`、`OSDCloudStepId`、`OSDCloudStepType`、`OSDCloudStepIndex`、`OSDCloudStepTimeoutSeconds`；需要跨 step 傳遞資料時，請明確讀寫 `OSDCloudInstallStatePath` 指向的 JSON，不要依賴 stdout pipe 或同一個 PowerShell process memory。`run.ps1` 與 `install.ps1` 都應設定 `$ErrorActionPreference = 'Stop'`，並在失敗時 `throw` 或 `exit <non-zero>`；runner 會標記為 failed，缺少腳本會標記為 missing，timeout 會標記為 timed_out，且第一個失敗步驟就停止 sequence。`windows-apps-error` 會把失敗 step 的 `stepIndex`、`stepType`、`stepId`、`reason`、`timeoutSeconds`、step log path 與 stdout/stderr tail 回傳到 host status JSON/JSONL，讓技術人員可在 server 端 Web/System Log/status evidence 看到錯誤摘要；完整 log 仍保留在 client。若中文 `Write-Host` 在 redirected log 內顯示亂碼，通常是 Windows PowerShell stdout/stderr redirect encoding 問題；要保留可讀中文內容時，腳本應自行用 `[System.IO.File]::WriteAllText(..., [System.Text.Encoding]::UTF8)` 或明確 `-Encoding UTF8` 寫入自己的 log/輸出檔。範例（建立暫存防火牆規則）：
...
`windows-apps-start` / `windows-apps-finished` 代表 SetupComplete 正在安裝 `C:\ProgramData\OSDCloud\Apps` 內的 client app payload，並在同一個 `Install-Apps.ps1` run 內執行 `C:\ProgramData\OSDCloud\Scripts` 內被 profile 選中的 custom scripts。若 installer 或 custom script 回傳非成功碼、缺檔或 timeout，會送出 `windows-apps-error` 與 `windows-setupcomplete-error`。client 端完整 log 在 `C:\Windows\Temp\osdcloud-logs\apps-install.log`、`C:\Windows\Temp\osdcloud-logs\install-sequence-summary.json`、`C:\Windows\Temp\osdcloud-logs\install-sequence-state.json`、`C:\Windows\Temp\osdcloud-logs\install-sequence\*.log`、`C:\Windows\Temp\osdcloud-logs\custom-scripts-summary.json` 和各軟體自己的 log；host status JSON 保存 stdout/stderr/transcript tail 與失敗 step 摘要，供 server 端快速判讀。

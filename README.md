...
這一段是給另一台 Windows host 從 Git clone 接手部署用的最短路徑。Repo 可以 clone 到任意資料夾；實際 PXE/OSDCloud runtime 仍固定使用 `C:\OSDCloud`。Git 只保存文件、設定、可審查腳本與 `osdcloud-assets` mirror，不提交大型 runtime artifact。
...
- Git 與 Git LFS；本 repo 的 MSI installer 由 LFS 管理。
...
在已驗證 host 上先匯出大型 runtime artifact bundle：

```powershell
.\tools\Export-DeploymentServerBundle.ps1 -Force -CreateZip
```

把產生的 `deployment-server-bundle` 資料夾或 `deployment-server-bundle.deployment-server.zip` 搬到新 host 的 repo root。新 host `git clone` 後可直接雙擊：
...
此 bootstrap 會還原 `C:\OSDCloud`、驗證大型 artifact size/SHA-256、執行 `git lfs pull` / `npm install` / `npm test` / `npm run smoke`、把 endpoint 同步到預設 `LAN 192.168.88.1/24`、跑 `npm run server:preflight`，最後啟動 Web console。它不會自動啟動 DHCP/TFTP/HTTP deployment services；實際 PXE 部署仍要在 Web console 確認真實 LAN DHCP server 已停用後手動 `Start all services`。

若沒有 bundle，或 bundle 缺少 `osdcloud-assets\manifest.json` 內列出的大型 artifact，bootstrap 會 fail closed 並列出缺檔，不會改 endpoint 或啟服務。要覆蓋預設 endpoint，可傳入 `-InterfaceAlias`、`-ServerIp`、`-PrefixLength`；只有明確加上 `-ConfigureNic` 時才會修改 Windows NIC IP。

### 2. Clone repo 並確認 Web console 可跑
...
若使用上方一鍵 bootstrap，`git lfs pull`、`npm install`、`npm test`、`npm run smoke` 會由 bootstrap 代跑；本段保留作為手動驗證流程。

### 3. 準備 live `C:\OSDCloud`

`git clone` 本身不能直接部署，因為下列大型或上游檔案不進 Git：Windows ESD/WIM/ISO、`boot.wim`、`wimboot`、Windows boot binaries、iPXE binaries、timing logs 與 screenshot。新 host 必須先讓 `C:\OSDCloud` 具備可執行 runtime。
...
然後另外還原 `osdcloud-assets\manifest.json` 中 `excludedArtifacts` 列出的 artifact。部署前至少要存在：
...
若 active Windows image 不存在，先在 Web console 的 `OS Image Cache` 用 Download catalog 或 Import Local 把 Windows 11 Pro 25H2 zh-TW ESD/WIM 加到 host cache，再手動 `Set active`。不要讓 WinPE client 在部署中下載 Windows。

### 4. 設定本次 PXE service endpoint

若新 host 使用目前規劃的實體 LAN：

```powershell
.\tools\Set-IpxePhysicalNic.ps1 -InterfaceAlias 'LAN' -ServerIp '192.168.88.1'
```

這只設定 Windows NIC，不會改 live `boot.ipxe` 或 WinPE 內嵌 endpoint。接著啟動 Web console：
...
在 Web console 先執行 `Select service interface`，選本次要服務 PXE client 的 NIC/IP。這會停止 running services、寫回 `config\osdcloud-tui.json`、重算 DHCP pool/router、更新 live `boot.ipxe`、提交 WinPE endpoint 到 `boot.wim`、同步 SMB firewall，並刷新 `osdcloud-assets`。

若使用一鍵 bootstrap，endpoint sync 已在 bootstrap 中以 `LAN 192.168.88.1/24` 預設值完成；進入 Web console 後仍要確認 endpoint bar 與 preflight 結果是否符合本次實際網路。
...
7. 實體 client 從 UEFI IPv4 PXE 開機，不插 USB、不掛 ISO。
8. 在 `Client Fleet`、`Validation`、`System Log` 觀察到 `winpe-start`、`smb-mounted`、`osdcloud-start`、`apply-image`、`osdcloud-finished`、`rebooting`、`windows-setupcomplete-*`、`windows-desktop-ready`。
9. 完成後停止 DHCP/TFTP/HTTP services。

目前 repo 的 `config\osdcloud-tui.json` 可能保留上一輪 VM regression 或其他 endpoint 快照。新主機第一次實體部署前，不要直接相信 committed endpoint；一定要在 Web console 重新選 service interface 並通過 preflight。
...
13. Windows 第一次開機後應自動登入 `davis` 桌面，Web/TUI 最終應收到 `windows-desktop-ready`。
14. 完成後在 Web console 停止 DHCP/TFTP/HTTP services，避免 host DHCP 留在網段上。

選 `Select service interface` 時，Web/TUI 會同步：
...
| Web/TUI 停在 `awaiting-windows` | 檢查 SetupComplete status URL、desktop-ready scheduled task、Windows 網路是否能連回 service IP |
...
2. Host service IP 必須存在於一張已啟用的 IPv4 介面上；介面名稱和 IP 每次以 Web/TUI 選取結果為準。
3. Host 啟動 PXE helper：PowerShell DHCP、PowerShell TFTP、Node HTTP server。
4. 實體筆電從 UEFI IPv4 PXE 開機，不使用 USB/ISO。
5. UEFI PXE client 透過 DHCP 拿到目前 Web/TUI config 範圍內的 lease、設定中的 gateway/router、DNS 與第一階段 boot file `ipxeboot/x86_64-sb/snponly.efi`。若測試目標需要走 upstream gateway，先在 endpoint/config 中設定清楚並重新同步。
...
5. 在 Web console 用 `Edit active` 勾選 active profile 要部署的軟體並存檔。Web console 會先停止 running 的 HTTP/TFTP/DHCP service，保留 profile 的 id/name/description 與未知欄位，只更新 `software` 清單，清空 live `Apps` 後立即發佈新 payload。TUI 仍保留同等 profile 操作作為備援。
...
9. 下一次實體 iPXE 部署時，Web/TUI 應看到 `windows-apps-start` 和 `windows-apps-finished`。若失敗，檢查 client 的 `C:\Windows\Temp\osdcloud-logs\apps-install.log` 和軟體自己的 log。
...
- 回到實體筆電前必須切回 Web/TUI 選定的實體 service interface / service IP
...
Desktop-ready reporter 會等到 `windows-desktop-ready` 成功 POST 到 host 後才移除 scheduled task；如果 Windows 桌面先出現但網路尚未連上目前 host status endpoint，它會每 `5` 秒重試，最多 `30` 分鐘，避免 Web/TUI 永遠停在 `awaiting-windows`。`Send-Status` 必須在 HTTP POST 或 WebClient fallback 成功後回傳 `$true`，否則 reporter 會把 HTTP `204` 當成未完成並每 5 秒重送相同 `windows-desktop-ready`，直到 30 分鐘 deadline。
...
- 若要切換本次要安裝的 client software 組合，使用 `Select deployment profile`；Web/TUI 會停止 running services，寫回 active profile，並只發佈該 profile 選中的 `Apps` payload
...
- 選擇新介面時，HTTP/TFTP/DHCP 任一服務若正在 running，Web/TUI 會先要求停止服務再更新 endpoint
...
- `dhcp.reservations` 可針對已知實體 client MAC 固定派發 IP；切到新 endpoint 時，Web/TUI / `Set-OsdCloudIpxeEndpoint.ps1` 會移除不在新 subnet 內的 reservation，避免沿用舊 `192.168.100.x` 或 vSwitch 位址
...
- 實體筆電從 UEFI IPv4 PXE 開機後，在 Web console 內看 Client Fleet、Validation、System Log；TUI 的 Clients、Client Detail、Logs、Validation 可作為備援
...
這個 repo 可以 clone 到任意資料夾；Git 追蹤文件、流程設定，以及從 `C:\OSDCloud` 同步出來的可讀部署資產。實際部署仍以 `C:\OSDCloud` 為執行位置；repo 的作用是保存可審查、可比較、可重建的腳本與 manifest。新 host clone 後必須先完成上方「新主機 Clone 後啟動流程」，尤其是還原 `C:\OSDCloud` 的大型 excluded artifacts，才可開始實體部署。
...
`OSDCloudDesktopReadyReport` 使用 any-user logon trigger 並以 SYSTEM 執行。不要把 scheduled task trigger 綁到 `$env:COMPUTERNAME\davis`；SetupComplete 階段可能尚無法穩定解析本機帳號 SID，會造成 `HRESULT 0x80070534`，導致 Web/TUI 停在 `windows-setupcomplete-finished`。

...
目前穩定版為 `v0.6.7`，永久保留於 tag `v0.6.7` 與 `release/v1`。v2 在 `codex/v2-rewrite` 開發，版本為 `2.0.0-alpha.1`；完成 MSI、migration、VM、實機與 release-readiness matrix 前不可取代 `master`。
...
目前 alpha 已完成 pipe token/schema 與 resource coordinator；執行路徑仍透過 legacy Controller，SQLite 尚未成為完整 live source of truth。實際 Windows service SID 的 pipe DACL、native adapters 與 retention/evidence wiring 仍列為 release blocker。

完整依賴方向與 ownership 見 [v2 architecture](docs/agent-reference/v2-architecture.md)。正式技師手冊只有 [Winception Deployment Manual](docs/winception-operations-manual.html) 一份。
...
產生 unsigned 內部測試 package：

```powershell
.\tools\v2\Build-WinceptionV2Package.ps1
```

加上 `-BuildMsi` 會編譯 MSI。正式發行必須以組織 code-signing certificate 執行 `Sign-WinceptionPackage.ps1` 並通過 fresh install、repair、upgrade rollback 與 uninstall-preserves-data 測試；未簽章只能稱為內部測試版。
...
- [正式操作手冊](docs/winception-operations-manual.html)
...
The stable line is `v0.6.7`, preserved by tag `v0.6.7` and branch `release/v1`. v2 (`2.0.0-alpha.1`) is developed on `codex/v2-rewrite` and must not replace `master` until signed MSI, migration, VM, physical-laptop and release-readiness acceptance are complete.
...
The alpha implements pipe authentication/schema validation and the resource coordinator, while execution still delegates to the legacy Controller and SQLite is not yet the complete live source of truth. Explicit service-SID pipe ACL enforcement, native adapters, and retention/evidence wiring remain release blockers.
...
Use the [official deployment manual](docs/winception-operations-manual.html), [release evidence matrix](TEST-RESULT.md), [v2 architecture reference](docs/agent-reference/v2-architecture.md), and [support policy](SUPPORT.md) as the authoritative product documentation.

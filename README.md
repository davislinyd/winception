...
4. 發佈後 `selected-profile.json` 會寫出 `installSequence: [{type, id}]`，並保留 `selectedSoftware` / `software` 供 client 端與診斷摘要使用。WinPE shutdown 會把 `Scripts` 子樹一併複製到 client 端 `C:\ProgramData\OSDCloud\Scripts`，SetupComplete 透過 `Install-Apps.ps1` 照 `installSequence` 執行 `Apps\<softwareId>\install.ps1` 或 `Scripts\<scriptId>\run.ps1`。

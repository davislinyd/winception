...
目前穩定版為 `v0.6.7`，永久保留於 tag `v0.6.7` 與 `release/v1`。v2 在 `codex/v2-rewrite` 開發，版本為 `2.0.0-alpha.4`；完成 MSI、migration、VM、實機與 release-readiness matrix 前不可取代 `master`。
...
The stable line is `v0.6.7`, preserved by tag `v0.6.7` and branch `release/v1`. v2 (`2.0.0-alpha.4`) is developed on `codex/v2-rewrite` and must not replace `master` until signed MSI, migration, VM, physical-laptop and release-readiness acceptance are complete.

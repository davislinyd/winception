...
- Acceptance implementation/detailed bilingual operator contract is docs/acceptance.md. Source/UI drafts, collector/client/report-only ticket, test-only profile gate/MAC-scoped boot credentials, router DHCP/ownership/bootstrap/network rounds, CI matrix and manuals are complete for Source review. Source committed as dae06b2c71e1f141d7c26d2a2d8caf3f93442afa; only subsequent evidence/handoff edits remain.
...
### Active failed bootstrap — must resolve before another run

- Elevated runner PID16072 (ordinary Get-Process/CIM cannot see elevated process; use elevated snapshot helper). Started23:06, reached PXE23:10. MAC00155D6C6580 leased192.168.177.200; boot-session403 outside acceptance scope, no envelope/Fleet/install. Snapshot .ai/acceptance-bootstrap-snapshot.json confirms PID; last snapshot23:07.
- At23:11 services stopped through API; at23:12 owned router stopped Off, .ai/acceptance-router-stopped.json. All six VMs Off, Fleet0, HTTP/TFTP/DHCP/torrent stopped. Old runner still waits Fleet until ~00:10 (+finally restore/sync), owns Global mutex. Do NOT start next run or installed reload while it waits. No deployment currently running; cleanup remains Pending, original endpoint/profile not restored yet.
- Automatic approval review rejected the command containing forced Stop-Process PID16072 (no detailed reason returned). Do not bypass it. Explicit user approval to terminate this exact owned idle runner is pending; otherwise wait its natural 60-minute deadline and finally cleanup. Terminal failure must retain current evidence, never count old result.json as latest.
- Evidence root C:\OSDCloud\HostTools\State\lab\evidence\acceptance-router-bootstrap-20260914; .ai/acceptance-router-bootstrap.log. Existing result.json is stale23:05 Blocked URI guard; current round files prove actual later run. State backup current150620-016. Original public state is state-after-start.json DIRECT state object (not .state); originalprofileIZVZO7PU, testprofileSE50433G. Cleanup must restore original profile/delete test profile, endpoint177.1 Server200-250/gateway1/DNS1.1.1.1,8.8.8.8, Secureboot default, Clear Fleet, router Clean and all five default firmware/checkpoints.
- Source MAC bug fixed: scope read after restore; deterministic owned VM GUID MAC, collision guard; next runner service-stop causes immediate failed wait/finally. Focused26/26 passed .ai/acceptance-round-mac-tests.log. These Source edits DO NOT affect old loaded runner. No automatic deployment retry; validate/commit before fresh idle guard/bootstrap.
...
2. Refresh installed host idle/API/NIC/VM state. Current read-only snapshot: operationfalse/fleet0/services0; AutoLab192.168.177.1, activeIZVZO7PU/image25H2, installed workspace runtimeC:\OSDCloud; auth.requiredfalse. Acceptance installed reload completed at22:59 from dae06b2; protected State backup HostTools-State-20260914-145854-135. HTTP source hash matches. Endpoint Sync completed; Preflight29/29; published WinPE hash EFE3AFE948B177BC8624A4EA7B8E20D66015F3F021DF2FA1ECA3BB0AC8BF7CE4. CLI79303 ended. Lab prerequisite passed .ai/acceptance-lab-prerequisite.json/log. Router Create resumed successfully, owned VM Off with LAN/4GiB/SB+TPM/Clean checkpoint. Initial Bootstrap failed before mutation because relative config resolved against native process directory; Resolve-Path fix + structured error stdout verified24/24. Elevated Bootstrap ValidateOnly now Passed. No DHCP/deployment started yet.

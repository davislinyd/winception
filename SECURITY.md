...
The security boundary assumes a managed Windows 11 host, local administrator-controlled installation, signed MSI, State ACLs, loopback management by default, and explicit trusted HTTPS for LAN management. A clone, unsigned package, disabled ACL, plaintext secret, arbitrary PowerShell path, expired certificate, skipped release test or unverified runtime is not a supported production configuration.

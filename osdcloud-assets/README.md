...
The USB-specific files are source templates only. `New-WinceptionUsbInstaller.cmd` copies the current live boot media into HostTools State staging, injects these templates into the staged `boot.wim`, and emits a sensitive USB/ISO snapshot. It does not replace `Startnet.cmd` or `Start-OSDCloud-iPXE.ps1` in live media.
...
3. Download or regenerate every required `config\runtime-artifacts.json` entry that is needed for the selected path. For physical PXE deployment this includes at least:
...
C:\OSDCloud\PXE-TFTP\bootmgfw.efi
C:\OSDCloud\PXE-TFTP\Boot\BCD
C:\OSDCloud\PXE-TFTP\Boot\boot.sdi
C:\OSDCloud\PXE-TFTP\sources\boot.wim
C:\OSDCloud\Media\OSDCloud\OS\<selected-image>.wim
```

The `PXE-TFTP` rows are the `secureboot` boot-mode staging produced by `tools\Publish-SecureBootTftp.ps1`, which `Prepare runtime` and endpoint sync run automatically. The `boot.wim` paths are all required: `Media\sources\boot.wim` is the source WinPE image that endpoint sync can mount/update, `PXE-HttpRoot\osdcloud\boot.wim` is the published file that iPXE downloads in `ipxe` mode, and `PXE-TFTP\sources\boot.wim` is the hardlink to the published file that Windows Boot Manager downloads over TFTP in `secureboot` mode. They may be hardlinks or separate files depending on the rebuild path, but readiness and preflight must be able to find a valid image for every role.
